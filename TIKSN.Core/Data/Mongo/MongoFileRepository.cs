...
namespace TIKSN.Data.Mongo
{
    public class MongoFileRepository<TIdentity, TMetadata> : IFileRepository, IFileRepository<TIdentity>, IFileRepository<TIdentity, TMetadata> where TIdentity : IEquatable<TIdentity>
    {
        ...
        public MongoFileRepository(IMongoDatabaseProvider mongoDatabaseProvider, string bucketName)
        {
            ...
            _bucket = new GridFSBucket<TIdentity>(database, new GridFSBucketOptions
            {
                BucketName = bucketName
            });

            _bucketRaw = new GridFSBucket(database, new GridFSBucketOptions
            {
                BucketName = bucketName
            });
        }
        ...
        {
            var fileInfo = await _bucket.Find(Builders<GridFSFileInfo<TIdentity>>.Filter.Eq(item => item.Filename, path)).SingleAsync(cancellationToken);

            var content = await _bucket.DownloadAsBytesAsync(fileInfo.Id, null, cancellationToken);
        ...
        {
            var content = await _bucket.DownloadAsBytesAsync(id, null, cancellationToken);

            var fileInfo = await _bucket.Find(Builders<GridFSFileInfo<TIdentity>>.Filter.Eq(item => item.Id, id)).SingleAsync(cancellationToken);
        ...
        public async Task<IFileInfo<TIdentity, TMetadata>> DownloadOnlyMetadataAsync(TIdentity id, CancellationToken cancellationToken)
        {
            ...
            return new FileInfo<TIdentity, TMetadata>(id, fileInfo.Filename, BsonSerializer.Deserialize<TMetadata>(fileInfo.Metadata));
        }
        ...
        public async Task<IFile<TIdentity, TMetadata>> DownloadWithMetadataAsync(TIdentity id, CancellationToken cancellationToken)
        {
            ...
            return new File<TIdentity, TMetadata>(id, fileInfo.Filename, BsonSerializer.Deserialize<TMetadata>(fileInfo.Metadata), content);
        }
