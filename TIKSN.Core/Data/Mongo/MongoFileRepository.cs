...
namespace TIKSN.Data.Mongo
{
        ...
        IFileRepository<TIdentity, TMetadata> where TIdentity : IEquatable<TIdentity>
    {
        ...
        {
            var fileInfo = await this._bucket
                .Find(Builders<GridFSFileInfo<TIdentity>>.Filter.Eq(item => item.Filename, path))
                .SingleAsync(cancellationToken);

            await this._bucket.DeleteAsync(fileInfo.Id, cancellationToken);
        }
        ...
        public async Task<IFile> DownloadAsync(string path, CancellationToken cancellationToken)
        {
            ...
            var content = await this._bucket.DownloadAsBytesAsync(fileInfo.Id, null, cancellationToken);
        ...
        {
            var fileInfo = await this._bucket.Find(Builders<GridFSFileInfo<TIdentity>>.Filter.Eq(item => item.Id, id))
                .SingleAsync(cancellationToken);
        ...
        public async Task<IFile<TIdentity, TMetadata>> DownloadWithMetadataAsync(TIdentity id,
            CancellationToken cancellationToken)
        {
            var content = await this._bucket.DownloadAsBytesAsync(id, null, cancellationToken);
        ...
        public Task<bool> ExistsByIdAsync(TIdentity id, CancellationToken cancellationToken) => this._bucket
            .Find(Builders<GridFSFileInfo<TIdentity>>.Filter.Eq(item => item.Id, id)).AnyAsync(cancellationToken);
