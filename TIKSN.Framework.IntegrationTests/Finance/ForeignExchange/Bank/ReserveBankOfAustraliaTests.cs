...
public class ReserveBankOfAustraliaTests
{
    ...
    public ReserveBankOfAustraliaTests(FrameworkCoreServiceProviderFixture fixture)
    {
        ArgumentNullException.ThrowIfNull(fixture);

        this.bank = fixture.GetRequiredService<IReserveBankOfAustralia>();
    ...
    public async Task ConversionDirection001()
    {
        ...
        var afterInDollar =
            await this.bank.ConvertCurrencyAsync(beforeInPound, australianDollar, this.timeProvider.GetUtcNow(),
    ...
    public async Task ConvertCurrency004()
    {
        ...
        foreach (var pair in currencyPairs)
        {
            ...
            _ = await
                new Func<Task>(async () =>
                        await this.bank.ConvertCurrencyAsync(before, pair.CounterCurrency,
                            this.timeProvider.GetUtcNow().AddDays(-20d),
                            cancellationToken: TestContext.Current.CancellationToken))
                    .ShouldThrowAsync<ArgumentException>();
        }
    }
    ...
    public async Task ConvertCurrency005()
    {
        ...
        var before = new Money(armenianDram, amount: 10m);

        _ = await
            new Func<Task>(async () =>
                    await this.bank.ConvertCurrencyAsync(before, belarusianRuble, this.timeProvider.GetUtcNow(),
                        cancellationToken: TestContext.Current.CancellationToken))
                .ShouldThrowAsync<ArgumentException>();
    }
    ...
    {
        var rates = await this.bank.GetExchangeRatesAsync(this.timeProvider.GetUtcNow(),
            cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task GetCurrencyPairs004()
        => await
            new Func<Task>(async () =>
                    await this.bank.GetCurrencyPairsAsync(this.timeProvider.GetUtcNow().AddMinutes(10),
                        cancellationToken: TestContext.Current.CancellationToken))
                .ShouldThrowAsync<ArgumentException>();
    ...
    public async Task GetCurrencyPairs005()
        => await new Func<Task>(async () =>
                await this.bank.GetCurrencyPairsAsync(this.timeProvider.GetUtcNow().AddDays(-20),
                    cancellationToken: TestContext.Current.CancellationToken))
            .ShouldThrowAsync<ArgumentException>();
    ...
    public async Task GetExchangeRate003()
    {
        ...
        {
            _ = await new Func<Task>(async () =>
                    await this.bank.GetExchangeRateAsync(pair, this.timeProvider.GetUtcNow().AddDays(-20d),
                        cancellationToken: TestContext.Current.CancellationToken))
                .ShouldThrowAsync<ArgumentException>();
        }
    }
